...
void Model::Stage(ID3D12Device5* const device)
{
	...
	m_blas->AddStagedGeometry(m_vertexBuffer->GetHeapGPUVirtualAddress(), DXGI_FORMAT_R32G32B32_FLOAT, sizeof(Vertex), static_cast<UINT>(m_vertices.size()),
		m_indexBuffer->GetHeapGPUVirtualAddress(), static_cast<UINT>(m_indices.size()));
